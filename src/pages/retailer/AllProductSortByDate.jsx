...
import { add_product_to_cart, cancel_order_from_cart, delete_product_in_cart } from '../../redux/services/retailer/retailerHomepage.service';
import { AddProductToCart, deleteProductInCart } from '../../redux/slices/retailer/homepageSlice/allShopSlice';
import { ToastContainer, toast } from 'react-toastify';
import { useParams,location, useLocation } from 'react-router-dom';
import { useEffect } from 'react';

export default function AllProductSortByDate() {
  const dispatch = useDispatch();
  
  const { productByDateData } = useSelector((state) => state.getDataAllShop);
  ...
  useEffect(() => {
    ...
    // console.log("counterLocalStorage all products ",counterLocalStorage)
   ...
   const handleIncrement = async (productId) => {
    ...
    const product = productByDateData.find((item) => item.id === productId);
    ...
    <div>
      <div className="w-full grid lg:grid-cols-5 sm:grid-cols-3 grid-cols-2 lg:gap-11 gap-4">
        {/* <Carousel
          ...
          arrowLeft={() =>
            productByDateData && productByDateData.length > 9 ? (
              <img
